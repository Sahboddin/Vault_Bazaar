...
def sslcommerz_payment_gateway(request, order_id, user_id, grand_total):
    ...
    post_body['currency'] = "BDT"
    post_body['tran_id'] = unique_transaction_id_generator()
    post_body['success_url'] = f"https://vault-bazaar.onrender.com/order/success/{post_body['tran_id']}/{request.user.id}"
    post_body['fail_url'] = 'https://vault-bazaar.onrender.com/account/profile/'
    post_body['cancel_url'] = 'https://vault-bazaar.onrender.com/account/profile/'
    post_body['emi_option'] = 0
    post_body['cus_email'] = 'request.user.email'  # Retrieve email from the current user session
    post_body['cus_phone'] = 'request.user.phone'  # Retrieve phone from the current user session
    post_body['cus_add1'] = 'request.user.address'  # Retrieve address from the current user session
